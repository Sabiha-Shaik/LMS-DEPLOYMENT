package com.fileupload.service.LearningResourceService;

import com.fileupload.Exception.LearningResourceExceptions.BatchNotFoundException;
...
import com.fileupload.repository.LearningResourceRepository.BatchBucketMappingRepository;
...
public class S3Service {
	...
	public List<Resource> uploadFiles(List<MultipartFile> files, Long batchId, Long courseId, Long topicId,
		...
		for (int i = 0; i < maxSize; i++) {
			...
			try {
				...
				if (file != null) {
					...
					resourceIds.add(savedResource.getResourceId());
				}
		...
		updateProgressForUsersInBatch(batchId, resourceIds);
	...
	private void updateProgressForUsersInBatch(Long batchId, List<Long> resourceId) {
		String uri = batchModuleUri + "/batch-id/employees/" + batchId;
		ResponseEntity<List<Long>> response = restTemplate.exchange(uri, HttpMethod.GET, null, new ParameterizedTypeReference<List<Long>>() {});
		List<Long> allUsers = response.getBody();
